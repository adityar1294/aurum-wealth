...
export default function TasksTab({ clientId }: Props) {
  const { user } = useAuth();
  ...
  const save = async () => {
    const db = getClientDb();
    if (!user) return;
    const data = { title: form.title, description: form.description, priority: form.priority, status: form.status, dueDate: new Date(form.dueDate), clientId, rmId: user.uid };
    if (editing) {
  ...
  return (
    <div>
      <div className="flex-between" style={{ marginBottom: 20 }}>
        ...
        <button className="btn btn-primary btn-sm" onClick={openNew}><Plus size={14} /> Add Task</button>
      </div>

      {tasks.length === 0 ? (
        <div className="empty-state"><h3>No tasks yet</h3><p>Add a task to track action items for this client</p></div>
      ) : (
                ...
                return (
                  <tr key={t.id} className={overdue ? 'overdue' : ''}>
                    <td>
                      ...
                      {t.description && <div className="text-secondary" style={{ fontSize: 12 }}>{t.description}</div>}
                    </td>
                    ...
                    <td>
                      <div className="flex gap-8">
                        {t.status !== 'completed' && (
                          <button className="btn-icon" title="Mark complete" onClick={() => quickComplete(t)}><CheckCircle size={13} color="var(--accent-green)" /></button>
                        )}
        ...
        <div className="modal-overlay" onClick={() => setOpen(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="field"><label className="label">Title *</label><input className="input" value={form.title} onChange={(e) => set('title', e.target.value)} required /></div>
